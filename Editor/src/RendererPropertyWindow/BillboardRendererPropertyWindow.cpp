...
void BillboardRendererPropertyWindow::_initProperties(void)
{
	...
	Append(wxEnumProperty(PRNL_BILLBOARD_TYPE, PRNL_BILLBOARD_TYPE, mBillboardTypes));
	...
	Append(wxEnumProperty(PRNL_ORIGIN, PRNL_ORIGIN, mOrigin));

	// Rotation Type: List
	mRotationTypes.Add(RT_VERTEX);
	mRotationTypes.Add(RT_TEXTURE_COORDINATES);
	wxPGId pid = Append(wxEnumProperty(PRNL_ROTATION_TYPE, PRNL_ROTATION_TYPE, mRotationTypes));
