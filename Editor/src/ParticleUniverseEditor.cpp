...
#else
      wxString command = wxString::FromAscii("firefox ")+ url.c_str();
#endif
   ...
   std::cout<<" -------------------- LoadListOfTemplates 10"<<std::endl;
	for (it = names.begin(); it != itEnd; ++it)
	{
		...
		if (append)
		{
			...
			wxString particleSystemName = ogre2wx(*it);
	...
	wxCommandEvent foo = (wxCommandEvent)0;
	OnTemplatesClick(foo);
...
void ParticleUniverseEditorFrame::doLoadFile(void)
{
	...
	if (loadDialog.ShowModal() == wxID_OK)
	{
		...
		std::ifstream fpIn;
		fpIn.open(loadDialog.GetPath().ToAscii());
		if (fpIn)
		{
			while (!fpIn.eof())
			{
	...
	if (mTextCtrl)
	{
		if (!(mTextCtrl->IsEmpty() || mCurrentParticleScript.empty()))
		{
			if (mCurrentParticleScript != wx2ogre(mTextCtrl->GetValue()))
			{
               ...
               wxString wxCatStr = ogre2wx(categoryName);
               wxString wxTemStr = ogre2wx(newTemplateName);
					mListOfTemplates->addItem(wxCatStr, wxTemStr);
				}
			}
		}
	}
...
void ParticleUniverseEditorFrame::replaceTemplateName(const ParticleUniverse::String& templateName)
{
	...
	{
		ParticleUniverse::String categoryName = mCurrentParticleSystemForRenderer->getCategory();
      wxString wxCategoryName = ogre2wx(categoryName);
		mListOfTemplates->addItem(wxCategoryName, wxTemplateName);
	}
}
//-----------------------------------------------------------------------
